...
(function(window) {
  ...
  if (!telemetryService) {
    console.error('KDAM data service not found');
    return;
  }
  ...
  function triggerRefresh(isManual) {
    if (elements.btnManualRefresh) {
      ...
      setTimeout(() => {
        ...
        if (isManual) {
          state.refreshCountdown = 15;
          if (elements.refreshTimer) elements.refreshTimer.textContent = '15s';
        }
      }, 350);
    }
  }
  ...
  function renderSvgMap() {
    if (!elements.koreaSvgMap) return;
  ...
  function renderAll() {
    ...
    if (pinsGroup) {
      pinsGroup.innerHTML = renderMapPinsSvg();
      bindMapPinEvents();
    }
  }

  function renderMetrics() {
    if (!elements.valAvgRate) return;
  ...
  function renderBasinBars() {
    if (!elements.basinBarsContainer) return;
  ...
  function renderDroughtPills() {
    if (!elements.droughtPillsList) return;
  ...
  function renderEventsFeed() {
    if (!elements.eventsFeedList) return;
   ...
   */
  function openDamInspector(damId) {
    ...
    const pinsGroup = document.getElementById('map-pins-group');
    if (pinsGroup) pinsGroup.innerHTML = renderMapPinsSvg();
  }
  ...
  function initEventListeners() {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape' && elements.dialogInspector && elements.dialogInspector.open) {
        closeDamInspector();
      }
    });
    ...
    if (elements.btnModalClose) elements.btnModalClose.addEventListener('click', closeDamInspector);
    if (elements.modalBackdropClose) elements.modalBackdropClose.addEventListener('click', closeDamInspector);
  }
